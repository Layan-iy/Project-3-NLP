...
# Load custom dataset
def load_custom_dataset(max_samples=25000):
    ...
    train_reviews = []
    train_labels = []
    for text, label in zip(df['train_reviews'].iloc[:max_samples], df['train_labels'].iloc[:max_samples]):
    ...
    test_reviews = []
    test_labels = []
    for text, label in zip(df['test_reviews'].iloc[:max_samples // 5], df['test_labels'].iloc[:max_samples // 5]):
        review, sentiment = preprocess_text(text, label)
